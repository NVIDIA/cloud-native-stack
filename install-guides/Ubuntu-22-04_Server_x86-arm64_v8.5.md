# NVIDIA Cloud Native Stack v8.5 - Install Guide for Ubuntu Server
## Introduction
...
NVIDIA Cloud Native Stack v8.5 includes:
- Ubuntu 22.04 LTS
- Containerd 1.7.3
...
```
 sudo apt install -y -q kubelet=1.25.12-00 kubectl=1.25.12-00 kubeadm=1.25.12-00
```
...
```
sudo kubeadm init --pod-network-cidr=192.168.32.0/22 --cri-socket=/run/containerd/containerd.sock --kubernetes-version="v1.25.12"
```

Eecute the following command for `CRI-O` systems:

```
sudo kubeadm init --pod-network-cidr=192.168.32.0/22 --cri-socket=unix:/run/crio/crio.sock --kubernetes-version="v1.25.12"
```
...
Refer to the Helm 3.12.2 [release notes](https://github.com/helm/helm/releases) and the [Installing Helm guide](https://helm.sh/docs/using_helm/#installing-helm) for more information.
...
```
NAME             STATUS   ROLES                  AGE   VERSION
#yourhost        Ready    control-plane          10m   v1.25.12
#yourhost-worker Ready                           10m   v1.25.12
```
...
```
helm install --version 23.6.0 --create-namespace --namespace nvidia-gpu-operator nvidia/gpu-operator --set driver.version=535.86.10 --wait --generate-name
```
...
```
 helm install --version 23.6.0 --create-namespace --namespace nvidia-gpu-operator nvidia/gpu-operator  --set driver.rdma.enabled=true  --wait --generate-name
```
...
```
 helm install --version 23.6.0 --create-namespace --namespace nvidia-gpu-operator nvidia/gpu-operator --set driver.rdma.enabled=true,driver.rdma.useHostMofed=true --wait --generate-name 
...
```
helm install --version 23.6.0 --create-namespace --namespace nvidia-gpu-operator nvidia/gpu-operator --set gds.enabled=true
```
...
gpu-operator-1606173805 nvidia-gpu-operator         1               2023-04-14 20:23:28.063421701 +0000 UTC deployed        gpu-operator-23.6.0      23.6.0 
