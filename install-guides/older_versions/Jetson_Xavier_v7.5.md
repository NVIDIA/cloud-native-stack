<h1> NVIDIA Cloud Native Stack - v7.5 Install Guide for Jetson AGX Xavier or Jetson Xavier NX DevKit or Jetson Orin</h1>
...
```
 sudo kubeadm init --pod-network-cidr=10.244.0.0/16 --cri-socket=/run/containerd/containerd.sock --kubernetes-version="v1.24.14"
```

Execute the following command for `CRI-O` systems::

```
 sudo kubeadm init --pod-network-cidr=10.244.0.0/16 --cri-socket=unix:/run/crio/crio.sock --kubernetes-version="v1.24.14"
```
