<h1>NVIDIA Cloud Native Stack v6.2 - Install Guide for Ubuntu Server x86-64</h1>
<h2>Introduction</h2>
...
- NVIDIA GPU Operator 1.11.0
  ...
  - NVIDIA KubeVirt GPU Device Plugin: 1.1.2
- NVIDIA Network Operator 1.2.0
  - Mellanox MOFED Driver 5.6-1.0.3.3
...
```
 sudo kubeadm init --pod-network-cidr=192.168.0.0/16 --cri-socket=/run/containerd/containerd.sock --kubernetes-version="v1.23.7"
```
...
Refer to the Helm 3.8.2 [release notes](https://github.com/helm/helm/releases) and the [Installing Helm guide](https://helm.sh/docs/using_helm/#installing-helm) for more information.
...
NAME             STATUS   ROLES                  AGE   VERSION
#yourhost        Ready    control-plane,master   10m   v1.23.8
#yourhost-worker Ready                           10m   v1.23.8
```
...
```
 helm install --version 1.11.0 --create-namespace --namespace gpu-operator-resources nvidia/gpu-operator --wait --generate-name
```
...
```
 helm install --version 1.11.0 --create-namespace --namespace gpu-operator-resources nvidia/gpu-operator --set driver.rdma.enabled=true,driver.rdma.useHostMofed=true --wait --generate-name 
...
gpu-operator-1606173805 gpu-operator-resources         1               2022-04-03 20:23:28.063421701 +0000 UTC deployed        gpu-operator-1.11.0      1.11.0 
...
NAME            	NAMESPACE       	REVISION	UPDATED                                	STATUS  	CHART                 	APP VERSION
network-operator	network-operator	1       	2022-04-03 17:09:04.665593336 +0000 UTC	deployed	network-operator-1.2.0	v1.2.0
