<h1>NVIDIA Cloud Native Core v3.1(formely EGX Stack 3.1) for AWS - Install Guide for Ubuntu Server x86-64</h1>
<h2>Introduction</h2>
This document describes how to set up the NVIDIA Cloud Native Core on one or multiple GPU accelerated AWS instances to deploy AI applications via Helm charts from NGC.
...
- [Adding additional node to the NVIDIA Cloud Native Core](#Adding-additional-node-to-the-NVIDIA Cloud Native Core-Stack)
...
AWS G4 EC2 instances provide the latest generation NVIDIA T4 GPUs. To set up an NVIDIA Cloud Native Core G4 instance via the AWS console web UI, please follow the below steps. 
...
The GPU Operator validates the stack through the nvidia-device-plugin-validation pod and the nvidia-driver-validation pod. If both are completed successfully (see output from kubectl get pods --all-namespaces | grep -v kube-system), NVIDIA Cloud Native Core is working as expected. 
