<h1>NVIDIA Cloud Native Stack v8.4 for AWS - Install Guide for Ubuntu Server</h1>
<h2>Introduction</h2>
...
NVIDIA Cloud Native Stack v8.4 includes:
...
```
```
 sudo apt-get install -y -q kubelet=1.25.10-00 kubectl=1.25.10-00 kubeadm=1.25.10-00
```
```
...
```
sudo kubeadm init --pod-network-cidr=192.168.32.0/22 --cri-socket=/run/containerd/containerd.sock --kubernetes-version="v1.25.10"
```
...
#yourhost        Ready    control-plane          10m   v1.25.10
```
...
```
NAME             STATUS   ROLES                  AGE   VERSION
#yourhost        Ready    control-plane,master   10m   v1.25.10
#yourhost-worker Ready                           10m   v1.25.10
```
...
```
helm install --version 23.3.2 --create-namespace --namespace gpu-operator-resources --devel nvidia/gpu-operator --set driver.version=535.54.03 --wait --generate-name
```
