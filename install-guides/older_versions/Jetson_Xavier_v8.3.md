<h1> NVIDIA Cloud Native Stack - v8.3 Install Guide for Jetson AGX Xavier or Jetson Xavier NX DevKit or Jetson Orin</h1>
...
```
 sudo apt-get update
 sudo apt-get install -y -q kubelet=1.25.8-00 kubectl=1.25.8-00 kubeadm=1.25.8-00
...
```
 sudo kubeadm init --pod-network-cidr=10.244.0.0/16 --cri-socket=/run/containerd/containerd.sock --kubernetes-version="v1.25.8"
```

Execute the following command for `CRI-O` systems::

```
 sudo kubeadm init --pod-network-cidr=10.244.0.0/16 --cri-socket=unix:/run/crio/crio.sock --kubernetes-version="v1.25.8"
```
...
```
NAME             STATUS   ROLES                  AGE   VERSION
#yourhost        Ready    control-plane,master   10m   v1.25.8
