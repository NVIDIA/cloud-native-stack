<h1>NVIDIA Cloud Native Stack v9.2 for AWS - Install Guide for Ubuntu Server</h1>
<h2>Introduction</h2>
...
```
```
 sudo apt-get install -y -q kubelet=1.26.5-00 kubectl=1.26.5-00 kubeadm=1.26.5-00
```
```
...
```
sudo kubeadm init --pod-network-cidr=192.168.32.0/22 --cri-socket=/run/containerd/containerd.sock --kubernetes-version="v1.26.5"
```
...
#yourhost        Ready    control-plane          10m   v1.26.5
```
...
Execute the following command to download Helm 3.12.1: 
...
Execute the following command to download Helm 3.12.1 for `ARM` system : 
...
For additional information about Helm, refer to the Helm 3.12.1 [release notes](https://github.com/helm/helm/releases) and the [Installing Helm guide](https://helm.sh/docs/using_helm/#installing-helm) for more information. 
...
```
NAME             STATUS   ROLES                  AGE   VERSION
#yourhost        Ready    control-plane,master   10m   v1.26.5
#yourhost-worker Ready                           10m   v1.26.5
```
...
```
helm install --version 23.3.2 --create-namespace --namespace nvidia-gpu-operator --devel nvidia/gpu-operator --set driver.version=535.54.03 --wait --generate-name
```
...
nvidia-gpu-operator   gpu-operator-1590097431-node-feature-discovery-master-76578jwwt   1/1     Running     0          5m2s
nvidia-gpu-operator   gpu-operator-1590097431-node-feature-discovery-worker-pv5nf       1/1     Running     0          5m2s
nvidia-gpu-operator   gpu-operator-74c97448d9-n75g8                                     1/1     Running     1          5m2s
nvidia-gpu-operator   gpu-feature-discovery-6986n                                       1/1     Running     0          5m2s
nvidia-gpu-operator   nvidia-container-toolkit-daemonset-pwhfr                          1/1     Running     0          4m58s
nvidia-gpu-operator   nvidia-cuda-validator-8mgr2                                       0/1     Completed   0          5m3s
nvidia-gpu-operator   nvidia-dcgm-exporter-bdzrz                                        1/1     Running     0          4m57s
nvidia-gpu-operator   nvidia-device-plugin-daemonset-zmjhn                              1/1     Running     0          4m57s
nvidia-gpu-operator   nvidia-device-plugin-validator-spjv7                              0/1     Completed   0          4m57s
nvidia-gpu-operator   nvidia-driver-daemonset-7b66v                                     1/1     Running     0          4m57s
nvidia-gpu-operator   nvidia-operator-validator-phndq                                   1/1     Running     0          4m57s
