<h1>NVIDIA Cloud Native Stack v6.2 for AWS - Install Guide for Ubuntu Server x86-64</h1>
<h2>Introduction</h2>
...
```
sudo kubeadm init --pod-network-cidr=192.168.0.0/16 --cri-socket=/run/containerd/containerd.sock --kubernetes-version="v1.23.8"
```
...
#yourhost        Ready    control-plane          10m   v1.23.8
```
...
Execute the following command to download Helm 3.8.2: 
...
For additional information about Helm, refer to the Helm 3.8.2 [release notes](https://github.com/helm/helm/releases) and the [Installing Helm guide](https://helm.sh/docs/using_helm/#installing-helm) for more information. 
...
```
helm install --version 1.11.0 --create-namespace --namespace gpu-operator-resources --devel nvidia/gpu-operator --wait --generate-name
```
