<h1>NVIDIA Cloud Native Core - v2.0 Install Guide for Jetson Xavier NX DevKit</h1>

<h2>Introduction</h2>

This document describes how to set up the NVIDIA Cloud Native Core Version 2.0 on a Jetson Xavier NX DevKit to enable the deployment of AI applications via Helm charts from NGC. 
...
```
The NVIDIA Cloud Native Core works as expected if the get pods command shows the pod status as completed. You can also verify the successfull run of the cuda-samples.yaml by verifyinng that the output shows Result=PASS
