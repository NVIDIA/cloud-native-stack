# NVIDIA Cloud Native Stack v9.3 - Install Guide for Ubuntu Server
## Introduction
...
NVIDIA Cloud Native Stack v9.3 includes:
- Ubuntu 22.04 LTS
- Containerd 1.7.3
...
```
 sudo apt install -y -q kubelet=1.26.7-00 kubectl=1.26.7-00 kubeadm=1.26.7-00
```
...
```
sudo kubeadm init --pod-network-cidr=192.168.32.0/22 --cri-socket=/run/containerd/containerd.sock --kubernetes-version="v1.26.7"
```

Eecute the following command for `CRI-O` systems:

```
sudo kubeadm init --pod-network-cidr=192.168.32.0/22 --cri-socket=unix:/run/crio/crio.sock --kubernetes-version="v1.26.7"
```
...
#yourhost        Ready    control-plane          10m   v1.26.7
```
...
NAME             STATUS   ROLES                  AGE   VERSION
#yourhost        Ready    control-plane          10m   v1.26.7
#yourhost-worker Ready                           10m   v1.26.7
```
