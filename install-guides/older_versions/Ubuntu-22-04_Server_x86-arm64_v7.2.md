<h1>NVIDIA Cloud Native Stack v7.2 - Install Guide for Ubuntu Server</h1>
<h2>Introduction</h2>
...
NVIDIA Cloud Native Stack v7.2 includes:
- Ubuntu 22.04 LTS
- Containerd 1.6.10
...
- NVIDIA Network Operator 1.4.0
  - Mellanox MOFED Driver 5.8-1.0.1.1.2
...
```
wget https://github.com/containerd/containerd/releases/download/v1.6.10/cri-containerd-cni-1.6.8-linux-arm64.tar.gz
```
...
```
 sudo apt-get install -y -q kubelet=1.24.8-00 kubectl=1.24.8-00 kubeadm=1.24.8-00
```
...
#yourhost        Ready    control-plane          10m   v1.24.8
```
...
NAME             STATUS   ROLES                  AGE   VERSION
#yourhost        Ready    control-plane          10m   v1.24.8
#yourhost-worker Ready                           10m   v1.24.8
```
...
```
 ...
 helm install --version 1.4.0 -f ./network-operator-values.yaml -n network-operator --create-namespace --wait network-operator mellanox/network-operator
```
...
NAME            	NAMESPACE       	REVISION	UPDATED                                	STATUS  	CHART                 	APP VERSION
network-operator	network-operator	1       	2022-12-03 17:09:04.665593336 +0000 UTC	deployed	network-operator-1.4.0	v1.4.0
