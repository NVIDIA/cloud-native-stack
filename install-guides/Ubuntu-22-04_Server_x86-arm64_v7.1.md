<h1>NVIDIA Cloud Native Stack v7.1 - Install Guide for Ubuntu Server</h1>
<h2>Introduction</h2>
...
```
wget https://github.com/containerd/containerd/releases/download/v1.6.6/cri-containerd-cni-1.6.8-linux-amd64.tar.gz
```
...
```
 sudo kubeadm init --pod-network-cidr=192.168.0.0/16 --cri-socket=/run/containerd/containerd.sock --kubernetes-version="v1.24.6"
```
...
Execute the following command to download and install Helm 3.10.0 for `x86-64` system: 
...
Download and install Helm 3.10.0 for `ARM` system: 
...
```
 helm install --version 1.11.0 --create-namespace --namespace gpu-operator-resources nvidia/gpu-operator  --set driver.rdma.enabled=true  --wait --generate-name
```
...
```
kubectl patch ds/nvidia-driver-daemonset -n nvidia-gpu-operator -p '{"spec":{"template":{"spec":{"containers":[{"name":"nvidia-driver-ctr","image":"nvcr.io/nvidia/driver:515.65.01-ubuntu22.04"}]}}}}'
```
...
gpu-operator-1606173805 gpu-operator-resources         1               2022-10-03 20:23:28.063421701 +0000 UTC deployed        gpu-operator-1.11.0      22.09 
...
NAME            	NAMESPACE       	REVISION	UPDATED                                	STATUS  	CHART                 	APP VERSION
network-operator	network-operator	1       	2022-10-03 17:09:04.665593336 +0000 UTC	deployed	network-operator-1.3.0	v1.3.0
