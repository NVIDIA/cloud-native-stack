<h1>NVIDIA Cloud Native Core v5.1 - Install Guide for Ubuntu Server x86-64</h1>
<h2>Introduction</h2>
...
```
 sudo kubeadm init --pod-network-cidr=192.168.0.0/16 --cri-socket=/run/containerd/containerd.sock --kubernetes-version="v1.22.5"
```
...
```
 helm install --version 1.10.1 --create-namespace --namespace gpu-operator-resources nvidia/gpu-operator --set driver.version=470.103.01 --wait --generate-name
```
...
```
 helm install --version 1.10.1 --create-namespace --namespace gpu-operator-resources nvidia/gpu-operator  --set driver.rdma.enabled=true --set driver.version=470.103.01 --wait --generate-name
```
...
```
 helm install --version 1.10.1 --create-namespace --namespace gpu-operator-resources nvidia/gpu-operator --set driver.rdma.enabled=true,driver.rdma.useHostMofed=true --set driver.version=470.103.01 --wait --generate-name 
```
...
```
helm install --version 1.10.1 --create-namespace --namespace nvidia-gpu-operator --devel nvidia/gpu-operator --set driver.version=470-signed --wait --generate-name
```
