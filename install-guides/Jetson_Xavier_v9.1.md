...
```
 sudo kubeadm init --pod-network-cidr=10.244.0.0/16 --cri-socket=/run/containerd/containerd.sock --kubernetes-version="v1.26.3"
```

Execute the following command for `CRI-O` systems::

```
 sudo kubeadm init --pod-network-cidr=10.244.0.0/16 --cri-socket=unix:/run/crio/crio.sock --kubernetes-version="v1.26.3"
```
